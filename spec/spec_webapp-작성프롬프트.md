...
## ① 질의·요청 히스토리
- **2026-08-14**: "5-2절에서 지식재산권·표준화·인증기준은 **도표로 작성**. 특히 지식재산권은
  **특허명·개발자·등록번호·주요 내용·관련 이미지**가 포함되게." → `_프롬프트/5-2_*.md` 변형 A를
  개정: IP=표(`특허명|개발자|등록번호|주요 내용`), 표준화=표(`표준번호|표제|발간기관|적용 관련성`),
  인증=표(`인증/규정|기관|대상·적용범위|확보 현황`). **관련 이미지는 표 셀이 아니라 특허 표 바로
  아래** `![](URL)`+출처로 배치(표 셀 내 이미지는 그림으로 변환 안 됨 — [그림삽입](spec_hwpx-그림삽입.md)).
  `regen_prompt_styles` 재생성으로 모든 프로젝트 5-2 즉시 반영. 5-2를 참조이미지 게이트에 추가(웹조사 ON)한
  건 [그림삽입](spec_hwpx-그림삽입.md) 참조.
