...
- **2026-08-14** (원문 요지) "5-2절 지식재산권을 표로 쓰되 **특허별 관련 이미지(대표도면)** 포함." →
  참조이미지 게이트 기본값에 **`5-2` 추가**(`_reference_image_nids` → `{1-2,5-1,5-2}`)로 5-2 작성 경로에
  웹조사+`_REF_IMAGE_GUIDE` 부착. 가이드에 "특허 대표도면·개념도를 특허공보(KIPRIS·Google Patents)에서
  찾아 **해당 특허 표 바로 아래**(표 셀 밖) 문단에 삽입" 한 줄 추가. 표 형식·열 구성은
  [작성프롬프트](spec_webapp-작성프롬프트.md) 참조. **표 셀 내 이미지는 여전히 미변환**(아래 ④)이라
  이미지는 반드시 표 밖 바로 아래에 둔다.
...
- **대상 절(게이트)**: `claude_service._wants_reference_images(nid)` — 기본 `{"1-2","5-1","5-2"}`(동향·시장
  성격 절 + 5-2 지식재산권 특허 대표도면). 환경변수 `IMAGE_SECTIONS`(공백/쉼표 구분)로 재정의. 그 외 절은
  종전과 동일(이미지 미삽입). 5-2 BM형에도 게이트가 걸리나(nid 단위) 시장·경쟁 이미지에 무해.
- **적용 경로(둘 다)**:
