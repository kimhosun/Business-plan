...
- **프롬프트측 개선(권장 후속)**: rnd-write/convert 가 각 절의 **기존 템플릿 표 컬럼 구조를 컨텍스트로 받아**
  그 열에 맞는 md 표를 생성하게 하면, 흡수가 셀 1:1로 정확해지고 폼 병합·각주까지 보존된다. (현재는 AI가
  임의 컬럼을 만들어 재구성이 불가피.) `default_template_from_hwpx`/노드 `table_paths` 로 열 헤더를 뽑아
  프롬프트에 주입하는 방식 검토.
