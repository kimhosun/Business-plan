...
- **2026-08-14 (#94)** (원문) "7.1 표 7은 주관연구책임자 1개와 각 기관 공동연구 책임자 별로 1개씩
  있어야 해. 맞춰서 추가해주는데, 탭으로 눌러서 작성할 수 있도록 반영해줘." + 문답(3): 복제 범위=**책임자
  블록 전체**(인적사항·학력·경력·국가R&D실적·논문/저서·지식재산권·수상·국외지원 표 묶음), 실행=**버튼**,
  '탭으로 작성'=**표 셀 간 Tab 키 이동**(자동 기입 아님). → 7-1 '공동연구개발기관책임자' 프로필 블록을
  공동기관 수만큼 복제하는 버튼(구조편집) + ③ 입력 그리드 셀 Tab 이동. →
  [후속30](#2026-08-14-후속30--7-1-공동책임자-프로필-블록-복제--그리드-셀-tab-이동) 참조.
- **2026-08-14 (#93)** (원문) "5.4 절에 표 내용도 ai가 제안해줬으면해." → 5-4 경제적 성과 표(매출전망·
  투자계획·사업화전략)를 **절 전체 AI 일괄 제안**(`o 항목:` 골격 셀 포함, 수치는 추정치+[확인 필요]),
  표 툴바 `✨ 5-4 표 AI 제안` 버튼. →
  [후속29](#2026-08-14-후속29--5-4-경제적-성과-표-ai-일괄-제안골격-셀-포함) 참조.
- **2026-08-14 (#92)** (원문) "4.3절 기술기여도는 정부출연금에서 해당 기관 연구비 비율이야. 1년차부터
  모든 차년도가. 비영리는 부담 안 해도 돼. 반영해서 표지 작성하면 자동 기입되게." → 4-3 기술기여도 표를
  기관별 **A/E(정부출연금/총연구개발비, 비영리 100%)** 로 자동 채움 + 행 부족 시 기관수 반영 sync 버튼. →
  [후속28](#2026-08-14-후속28--4-3-기술기여도-표-자동-채움정부출연금총연구개발비-비영리-100) 참조.
...
### 2026-08-14 후속28 — 4-3 기술기여도 표 자동 채움(정부출연금/총연구개발비, 비영리 100%)

- **질의**(#92): "4.3절 기술기여도는 **정부출연금에서 해당 기관 연구비 비율**이야. 1년차부터 모든
  차년도가. **비영리는 부담 안 해도 돼**. 반영해서 표지 작성하면 자동 기입되게."
- **표 구조 진단**(s2/p390/t0, 전 프로젝트 표준 서식): 행=`연구개발 기관명`, 열=`매출액발생 1~5년차`
  (고정 5열, R&D 차년도 아님·기술료 대상 매출연차), 값=기술기여도(%). 예시 행 2개(AAAAA·BBBBB)만.
- **확정 사양**: 기술기여도 = **정부출연금(A) / 총연구개발비(E) × 100** (협약 비율, 기술료 산정 계수).
  `_funding_breakdown` 이 산출한 A·E 사용 → 영리는 유형별 정부지원율(대 50·중견 70·중소 80%),
  **비영리·대학·출연연·기타는 기관부담 0 → 100%**. 계수는 연차 무관 일정이므로 **매출연차 전 열에 동일 값**.
  - **필러**(`doc_fill._fill_contrib_43`, `_FILLERS` 에 추가): 표를 **4-3 노드 table_paths 로 한정**해
    찾는다(`contrib_table_path` — 1장 요약 내 동일 시그니처 중첩 복사본 s1/… 오탐 방지). 헤더 셀이
    `연구개발\n기관명` 2문단이라 공백 제거 후 시그니처(`기술기여도`+`매출액발생`+`기관명`) 매칭.
    기관별 A·E 합으로 A/E%(소수1자리) 계산해 각 기관 행 c0(기관명)·값 열 전부에 기입. 남는 예시 행은 비움.
    빌드/저장/표채우기 시 `doc_fill.apply` 로 매번 반영(멱등).
  - **행 부족 대응**(구조편집 sync 버튼): 표준 서식이 2행뿐이라 **3개 이상 기관은 행 모자람**. 8-x 예산
    표와 동일 패턴으로 **`POST /contrib/sync-rows`**(`_sync_contrib_rows`)가 `tables.add_row` 로 데이터
    행을 기관 수만큼 복제한 뒤 `doc_fill.apply`. 프런트 버튼 **'4-3 기술기여도 기관수 반영'**(정부출연금
    섹션, 8.1/8.2/8장 세부표 버튼 옆). 저비용 값채움은 자동, 위험한 행 복제만 버튼(수십 초·롤백).
- **검증**(5fb3cd5c 사본, 3기관): sync 2→3행 + apply → 한국선급(비영리)=100.0, 대우건설(대기업)=50.0,
  설계사(중소)=80.0, 매출연차 1~5 전 열 동일. `tables_filled=8`(신규 필러 포함, 회귀 0). main 라우트 등록·
  py_compile·JS 밸런스 OK. 사본 삭제.
- **한계**: A/E 는 유형별 정부지원율(`_GOV_RATIO`)로 결정 → 실제 기관부담 실적이 달라도 유형 기본율을
  따른다(제반사항에 정부출연금만 저장). 행 복제 후에도 매출연차는 5열 고정.

### 2026-08-14 후속29 — 5-4 경제적 성과 표 AI 일괄 제안(골격 셀 포함)

- **질의**(#93): "5.4절에 **표 내용도 AI가 제안**해줬으면." + 문답: **5-4 전용 버튼**(제반사항/5-4 절),
  매출·투자 표 수치는 **추정치 제안 + 근거 약하면 [확인 필요]**.
- **진단**: 표별 범용 'AI 채우기'는 **빈 칸만** 채워, 5-4 사업화 전략 표(s2/p458·p460)의 셀이 이미
  `o 사업화 형태 :`·`o 수요처 :` 같은 **라벨 골격**을 담고 있어 '빈 칸 아님'으로 건너뛰었다(기대효과만 채움).
  → 골격 셀도 채움 대상에 넣는 5-4 전용 제안이 필요.
- **확정 사양**:
  - **채움 대상 판정**(`main._is_econ_fillable`): 빈 셀 **또는** 모든 비공백 줄이 (콜론으로 끝나는 라벨)
    /(맨 불릿 `o ○ ㅇ - ·` 등)뿐인 **골격 셀**. 실제 값·머리글(구분·시장점유율(%)·(단위:…) 등)은 보호.
  - **엔드포인트**(`POST /nodes/{nid}/tables/ai-fill-econ`): 절 **전체 표**를 한 번에 처리. 각 표의
    grid + 채울 좌표(+골격 current)를 모아 `claude_service.econ_tables_suggest` 호출. 컨텍스트=절 제목+
    **제반사항 직렬화**(+RFP). 반환 `{edits:[{paths,text,table,row,col}]}`(미저장 — 프런트 검토 후 [표 저장]).
  - **프롬프트**(`_ECON_TABLE_SYSTEM`): 세 표 성격(매출·시장점유율 전망/투자계획/사업화 전략) 인지.
    골격 셀은 **항목 뒤에 값 채워 셀 전체 텍스트로** 반환(라벨·순서·개행 보존). 수치는 추정치+근거 약하면
    `[확인 필요]`, 표 내 관계(매출액≈판매량×단가, 자본적지출 합계, 연차 증가) 준수. 허용 좌표 밖 응답 폐기.
  - **프런트**: 표 툴바에 **`✨ 5-4 표 AI 제안`** 버튼(`btn-tables-ai-econ`), `loadTables` 에서
    `/^5-4($|-)/` 절에서만 노출. `runEconTablesFill` 이 edits 를 grid td 에 반영(골격/빈 칸 모두 덮어씀,
    헤더·수식 셀 제외) → dirty 표시, 검토 후 [표 저장]→[hwpx 빌드]로 문서 표에 반영.
- **검증**(5fb3cd5c 실데이터, 미저장): 채움 대상 53셀(표0 27·표1 18·표2 4·표3 4) 정확 분류(헤더 보호).
  실 LLM 호출 53셀 제안 — 수치 `2.5[확인 필요]`/매출 `3,600`, 사업화 전략 골격 완성 + **제반사항 참여기업
  (대우건설·설계사)을 표별로 정확 반영**. 라우트 등록·py_compile·JS 균형(HEAD 델타 동일) OK.
- **한계**: 수치는 추정치(사용자 검토 필수). 매출/투자 표의 '개발 종료 후 N년' 연도 머리글(`(   년)`)은
  보호(채움 대상 아님). 저장 전 dirty 로만 반영 — [표 저장] 전까지 문서 미반영. 표별 범용 AI 버튼은 유지.

### 2026-08-14 후속30 — 7-1 공동책임자 프로필 블록 복제 + 그리드 셀 Tab 이동

- **무엇/왜**: 7-1 연구책임자 절은 **주관책임자 프로필 1블록 + 공동책임자 프로필 1블록(예시, "※ 공동기관
  없으면 삭제")** 만 원본에 있어, 공동기관이 여럿이면 책임자별 표가 부족했다. 공동기관 수만큼 **공동책임자
  프로필 블록**을 복제하고, 늘어난 표를 ③ 입력 그리드에서 **Tab 키로 빠르게** 채우게 함(개인정보 비수록
  원칙상 값은 자동 기입하지 않음 — 사용자 수기). 문답 결정: 복제 범위=책임자 블록 전체, 실행=버튼,
  탭=셀 Tab 이동(자동 기입 아님).
- **블록 정의·경계**: 한 공동책임자 블록 = `공동연구개발기관책임자` 제목 문단부터 `참여연구자`(총괄) 문단
  **직전까지**의 섹션 직속 문단 구간. 그 안에 소제목+표 묶음(인적사항·학력·경력·국가R&D실적·논문/저서·
  지식재산권·수상·국외지원 = **블록당 8표**)과 빈 문단이 모두 포함된다. 경계·헤딩 판정은 **표를 품지 않은
  순수 제목 문단 텍스트**로만 하여(표 셀 텍스트 오탐 방지), `공동연구개발기관책임자`/`참여연구자` 부분일치.
  주관 블록은 손대지 않는다.
- **복제/제거**(`tables.rebuild_researcher_blocks(pid, target_n)`): 마지막 공동 블록을 템플릿으로
  섹션 직속 문단 구간을 lxml `deepcopy`(+`_regen_ids`) 해 참여연구자 총괄 **직전에** 순서대로 삽입, 초과분은
  구간째 제거. `_apply_structural_doc`(순수복원→문단블록 조작→재추출, 실패 시 롤백) 사용. **최소 1블록 보존**
  (target 0이면 삭제 안 함 — 템플릿 없으면 재추가 불가하므로). 표 개수가 바뀌면 **tree.json(절별 table_paths)을
  재생성**해 ③ 입력 그리드에 새 블록이 즉시 뜬다(구조편집 헬퍼는 tree 재생성을 안 하므로 이 함수가 직접 호출).
- **기관 수 산정**: 이름 있는 **역할=공동** 참여기관 수(`_sync_researcher_blocks`). 공동기관 0이면 미변경.
  값 채움(doc_fill) 없음 — 순수 구조 조정.
- **엔드포인트/버튼**: `POST /api/projects/{pid}/researcher/sync-blocks` → `{rebuilt}`. 표지/요약문 모달
  정부출연금 버튼줄에 **`7-1 공동책임자표 기관수 반영`**(`btn-researcher-sync`), 핸들러 `researcherSyncBlocks`
  (다른 sync 버튼과 동일 패턴, 저장 후 실행·상태줄). 현재 열린 절이 7-1 이면 `loadTables("7-1")` 로 즉시 갱신.
- **셀 Tab 이동**(`gridKeydown`): ③ 입력 표 그리드에서 **Tab=다음 셀 / Shift+Tab=이전 셀**(문서 순서 =
  행 끝→다음 행, 표 끝→다음 표). 이동 시 셀 내용 전체 선택(타이핑하면 덮어쓰기 = 빠른 채움). 그리드
  처음/끝에서는 기본 동작(밖으로). 종전 Enter=같은 열 아래 셀 이동은 유지.
- **구현/검증**(완료 2026-08-14):
  - [x] `backend/tables.py` `rebuild_researcher_blocks`(+경계 판정·deepcopy·tree 재생성).
  - [x] `backend/main.py` `POST /researcher/sync-blocks` + `_sync_researcher_blocks`(공동기관 수).
  - [x] `frontend/app.js` `API.researcherSyncBlocks`·`researcherSyncBlocks`·리스너·`gridKeydown` Tab 분기.
  - [x] `frontend/index.html` 버튼 `btn-researcher-sync`, 캐시버스트 `v=20260814f`.
  - [x] 검증(09754646 **복사본**, 실데이터 미변경): 7-1 table_paths 24→**40**(공동 1→3, 블록당 8표),
    3→3 재실행 무변경(멱등), 3→1 축소 시 **24 복귀**, hwpx 재오픈 무결성 OK. py_compile OK.
- **유의/한계**: (1) 복제 블록에 원본 ※주석("공동기관 없으면 삭제")·플레이스홀더(yy.mm 등)가 그대로 복사됨
  — 값은 사용자가 Tab 으로 덮어씀. (2) 각 블록에 **기관명 라벨 자동 기입은 안 함**(문답에서 자동 기입 대신 셀
  Tab 이동 선택). 필요 시 doc_fill 로 기관명만 채우는 건 후속. (3) 구조편집이라 수십 초 소요·별도 버튼.
  실행 중 서버가 옛 코드(라우트 없음)면 404 → **서버 재기동** 필요.
