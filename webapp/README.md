...
5. **입력** 패널: 원문을 쓰고 저장. 패널 **하단 채팅**에 재료·수정 지시를 적으면
   ...
   질문 내용과 **관련된 저장 스킬**이 자동으로 함께 적용된다(채팅 헤더 [🧰 스킬 자동적용] 토글로 끔/켬,
   적용된 스킬은 답변 아래 배지로 표시).
6. **변환** 패널: [변환] → 입력이 양식·문체대로 바뀌어 해당 절 노드에 매핑(before/after 표시).
7. 상단 **[hwpx 빌드]** → `output/final.hwpx` 생성, 다운로드 + PDF 미리보기 링크.

## 스킬 보관함 (🧰 스킬)

자주 쓰는 작성 규칙·노하우를 **스킬**로 저장해 두면, 작성 채팅에서 **질문과 관련된 스킬**이 자동으로
프롬프트에 붙는다(상단바 **[🧰 스킬]**).

- **이름 / 설명 / 트리거 키워드 / 적용 범위 / 본문**을 넣어 저장. *설명·트리거가 자동 선택의 기준*이므로
  "언제 쓰는 스킬인지"를 구체적으로 적을수록 잘 붙는다.
- 적용 범위: **자동**(질문과 관련될 때만) · **항상**(모든 대화) · **사용 안 함**.
- 저장소 `.claude/skills/*/SKILL.md` 를 **가져오기**로 그대로 담을 수 있다(형식이 같다).
- 모달 하단 **질문 미리보기**로 "이 질문이면 어떤 스킬이 붙는지"를 점수와 함께 확인할 수 있다.
- 선택은 모델 호출 없는 키워드 매칭이라 지연이 없다(상위 3개·본문 합계 12000자 이내).
- 저장 위치는 `webapp/data/skills/<slug>.md`(프로젝트 공용, git 미추적). 상세: [ARCHITECTURE.md](ARCHITECTURE.md),
  [../spec/spec_webapp-스킬보관함.md](../spec/spec_webapp-스킬보관함.md).
